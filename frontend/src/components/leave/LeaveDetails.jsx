  ...
  const [loading,setLoading]=useState(true)
   ...
   useEffect(() => {
     const fetchLeaveDetails=async()=>{
        try {
          const res=await API.get(`/leave/view/${id}`, {
            headers: {
    ...
    <div className="bg-white p-10 rounded-xl shadow-lg w-full max-w-2xl space-y-6">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
        ...
        <div className="flex-1 space-y-3 text-left">
          ...
          <p className="text-md">
            <strong>Rason:</strong> {employee.reason}
          </p>
          <p className="text-md">
            <strong>Department:</strong> {employee.department}
          </p>
          <p className="text-md">
            <strong>Start Date:</strong> {employee.startDate ?new Date(employee.startDate).toLocaleDateString() : ''}
          </p>
          <p className="text-md">
